...
//    }
    ...
    public static boolean maxTileExists(Board b) {
        ...
        for (int i = 0; i < b.size(); i++) {
            for (int k = 0; k < b.size(); k++) {
                if (b.tile(k, i) != null && b.tile(k, i).value() == MAX_PIECE) {
                    return true;
                }
            }
        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int i = 0; i < b.size(); i++) {
            for (int k = 0; k < b.size(); k++) {
                if (emptySpaceExists(b) || adjacentTiles(b, b.tile(k, i))) {
                    return true;
                }
            }
        }
    ...
    private static boolean adjacentTiles(Board b, Tile tile) {
        if (upLeftRightDown(b, tile)) {
            return true;
        }
        return false;
    }

    private static boolean upLeftRightDown(Board b, Tile tile) {
        if (validMove(b, tile.col() - 1, tile.row()) && b.tile(tile.col() - 1, tile.row()).value() == tile.value()) {
            return true;
        }
        if (validMove(b, tile.col() + 1, tile.row()) && b.tile(tile.col() + 1, tile.row()).value() == tile.value()) {
            return true;
        }
        if (validMove(b, tile.col(), tile.row() - 1) && b.tile(tile.col(), tile.row() - 1).value() == tile.value()) {
            return true;
        }
        if (validMove(b, tile.col(), tile.row() + 1) && b.tile(tile.col(), tile.row() + 1).value() == tile.value()) {
            return true;
    ...
    private static boolean validMove(Board b, int col, int row) {
